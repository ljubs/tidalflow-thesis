...
extern "C" int runTidalFlowCUDAInstanceWithBlockSize(const char *graphPath,
                                                     int source,
                                                     int sink,
                                                     int mode,
                                                     int blockSize,
...
static long long tideCycle(Graph *g, int source, int sink, const int *levelVer, int numLevels,
    ...
    for (int level = 0; level < numLevels; level++) {
        ...
        int numBlocks = (levelCount + blockSize - 1) / blockSize;
        highTide<<<numBlocks, blockSize>>>(
...
static __global__ void buildLevelGraphKernel(const int *ver, const int *dst, const int *capacity,
    ...
    int threadId = blockIdx.x * blockDim.x + threadIdx.x;
    if (threadId >= frontierSize) return;
    ...
    for (int e = ver[u]; e < ver[u + 1]; e++) {
        ...
        if (oldDist == -1) {
            int nextPos = atomicAdd(nextFrontierSize, 1);
            nextFrontier[nextPos] = v;
        }

        if (oldDist == -1 || oldDist == nextLevel) {
            int edgePos = atomicAdd(levelEdgeCount, 1);
            levelEdges[edgePos] = e;
        }
    }
}
...
static int buildLevelGraph(Graph *g, int source, int sink, int *levelVer,
                           ...
                           int *d_reach, int *d_writeCount, int *oldLevelVer,
                           int blockSize) {
    ...
    while (frontierSize > 0) {
        ...
        buildLevelGraphKernel<<<numBlocks, blockSize>>>(
    ...
    for (int lvl = level - 1; lvl >= 0; lvl--) {
        ...
        backwardReachKernel<<<numBlocks, blockSize>>>(
    ...
    for (int lvl = 0; lvl < level; lvl++) {
        ...
        if (count > 0) {
            int numBlocks = (count + blockSize - 1) / blockSize;
            compactLevelEdgesKernel<<<numBlocks, blockSize>>>(
...
static long long tidalFlowWithBlockSize(Graph *g, int source, int sink, int blockSize) {
    ...
    int *d_levelEdgeCount = NULL, *d_nextFrontierSize = NULL;
    ...
    while (1) {
        int hasLevel = buildLevelGraph(g, source, sink, levelVer,
                                       ...
                                       oldLevelVer, blockSize);
        if (!hasLevel) break;
        while (1) {
            long long flow = tideCycle(g, source, sink, levelVer, numLevels,
                                       d_src, d_dst, d_rev, d_capacity, d_flow,
                                       d_compactedLevelEdges, d_h, d_p, d_l, blockSize);
            if (flow == 0) break;
            maxFlow += flow;
        }
    }
...
long long tidalFlowCUDA(Graph *g, int source, int sink) {
    return tidalFlowWithBlockSize(g, source, sink, DEFAULT_BLOCK_SIZE);
}

extern "C" int runTidalFlowCUDAInstance(const char *graphPath,
                                        int source,
                                        int sink,
                                        int mode,
                                        double *totalTimeOut,
                                        long long *maxFlowOut) {
    return runTidalFlowCUDAInstanceWithBlockSize(
