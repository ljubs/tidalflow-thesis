 ...
 * it builds one bfs level graph, runs one tide cycle, then rebuilds.
 */
...
long long tidalFlowSeq_BASELINE_STATS(Graph *g, int source, int sink, TidalFlowSeqStats *stats) {
    ...
    while (hasLevelGraph) {
        phaseStart = tidalFlowSeqNowSeconds();
        maxFlow += tideCycleBaseline(g, source, sink, levelEdges, levelEdgeCount, h, p, l, stats);
        if (stats != NULL) {
...
long long tidalFlowSeq_BASELINE(Graph *g, int source, int sink) {
    return tidalFlowSeq_BASELINE_STATS(g, source, sink, NULL);
}
